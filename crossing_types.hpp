...
// crossing_types.hpp
//
// Data types for the input and output of the marsh crossing problem.
//
...
enum cell_kind { CELL_OK, CELL_THICKET};
...
public:
  ...
  : cells_(rows, std::vector<cell_kind>(columns, CELL_OK)) {
  ...
  // (0, 0) may only be CELL_OK. Other coordinates may be any kind.
  ...
  static grid random(coordinate rows, coordinate columns,
    ...
    for (unsigned i = 0; i < thicket_count; ++i) {
      ...
      result.set(p.row, p.column, CELL_THICKET);
      positions.pop_back();
    }
...
public:
  ...
  // try to step into a CELL_THICKET cell.
  bool is_step_valid(step_direction dir) const {
    ...
    return ((dir != STEP_DIRECTION_START) &&
            ...
            (setting_->get(row, column) != CELL_THICKET));
  }
